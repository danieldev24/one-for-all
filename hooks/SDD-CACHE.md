...
```
   ...
   `${CLAUDE_PROJECT_DIR}` resolves to the directory you launched Claude Code from. The snippet above works when the hooks live inside the same project. If you installed `one-for-all` elsewhere (e.g. as a shared plugin under `~/one-for-all`), replace `${CLAUDE_PROJECT_DIR}/hooks/...` with the absolute path to each script.
