...
if (hasJq) {
  ...
  if (!payload.message.includes('one-for-all loaded.')) {
    throw new Error('message is missing startup preface');
  }

  if (!payload.message.includes('# Using one-for-all')) {
    throw new Error('message is missing using-one-for-all content');
  }
} else {
